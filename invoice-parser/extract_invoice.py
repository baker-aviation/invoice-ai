...
def build_normal_messages(pdf_text: str) -> List[dict]:
    system = (
        ...
        "\n"
        "Fuel vendor (vendor.fuel_vendor):\n"
        "- This is the fuel CONTRACT vendor or fuel supplier — the company that supplies or brokers the fuel.\n"
        "- Common fuel vendors: Avfuel, World Fuel Services (WFS), EVO Fuels, Everest Fuel, AEG Fuels, Titan Fuels.\n"
        "- If the invoice is FROM a fuel vendor (e.g. Avfuel Corporation, World Fuel Services), set fuel_vendor to that company name AND use it as the main vendor.name too.\n"
        "- If the invoice is FROM an FBO (e.g. Signature, Atlantic Aviation, Jet Aviation) but mentions a fuel contract vendor or fuel supplier name anywhere on the document (header, footer, fuel line description, contract reference, logo), extract it as fuel_vendor.\n"
        "- Look for mentions like 'Contract Fuel', 'Supplied by', 'Fuel Program', fuel vendor logos, or vendor references near fuel line items.\n"
        "- If no fuel contract vendor is mentioned or the document has no fuel charges, set fuel_vendor to null.\n"
        "\n"
        "FBO invoice fields:\n"
