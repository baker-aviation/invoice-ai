...
export async function middleware(request: NextRequest) {
  ...
  const csrfExemptPaths = ["/api/aircraft/webhook", "/api/public/form-intake", "/api/foreflight/webhook", "/api/webhooks/graph-mail"];
  ...
  const publicApiPaths = ["/api/agents", "/api/vans/health", "/api/vans/diagnostics", "/api/invite", "/api/aircraft/webhook", "/api/foreflight/webhook", "/api/webhooks/graph-mail"];
  if (
    publicApiPaths.some((p) => request.nextUrl.pathname === p) ||
    request.nextUrl.pathname.startsWith("/api/cron/") ||
    request.nextUrl.pathname.startsWith("/api/jobs/backfill-prd") ||
