...
export async function POST(req: NextRequest) {
  ...
  // Call ops-monitor's seed endpoint on Cloud Run
  ...
  try {
    const res = await fetch(`${opsUrl}/jobs/seed_ics_sources`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    });
    const data = await res.json();
    if (!res.ok) {
      return NextResponse.json({ error: data.detail ?? "Seed failed" }, { status: res.status });
    }
