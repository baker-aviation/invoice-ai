...
export default function OpsMap({ adsbAircraft, flightInfo }: Props) {
  return (
    ...
    >
      ...
      {/* FlightAware-only aircraft (fallback for those not in ADS-B) */}
      {[...flightInfo.values()]
        .filter((fi) =>
          fi.latitude != null &&
          fi.longitude != null &&
          fi.status?.includes("En Route") &&
          !adsbAircraft.some((ac) => ac.tail === fi.tail)
        )
        .map((fi) => (
          <Marker
            key={`fa-${fi.tail}`}
            position={[fi.latitude!, fi.longitude!]}
            icon={adsbDivIcon(fi.heading ?? null, false)}
            zIndexOffset={1500}
          >
            <Tooltip permanent direction="top" offset={[0, -14]} className="van-label-tooltip">
              <span style={{ fontWeight: 700, fontSize: "10px", color: "#7c3aed" }}>
                {fi.tail}
              </span>
            </Tooltip>
            <Popup>
              <div className="text-sm space-y-1">
                <div className="font-bold text-purple-700">✈ {fi.tail}</div>
                <div className="text-[10px] text-purple-500 font-medium">FlightAware position</div>
                ...
                <div className="text-xs text-purple-600 font-medium">
                  Airborne{fi.altitude != null ? ` · ${fmtAlt(fi.altitude)}` : ""}
                </div>
                {fi.groundspeed != null && (
                  <div className="text-xs text-gray-600">
                    GS: {Math.round(fi.groundspeed)} kts{fi.heading != null ? ` · HDG: ${Math.round(fi.heading)}°` : ""}
                  </div>
                )}
                {fi.route_distance_nm && (
                  <div className="text-xs text-gray-500">Route: {fi.route_distance_nm} nm</div>
                )}
                {fi.diverted && (
                  <div className="text-xs font-semibold text-red-600">DIVERTED</div>
                ...
                <div className="text-xs text-gray-400">
                  {fi.latitude!.toFixed(4)}, {fi.longitude!.toFixed(4)}
                </div>
              </div>
            </Popup>
          </Marker>
