...
type FormData = {
  form: { title: string; description: string | null; questions: Question[] };
...
export default function CandidateFormPage() {
  ...
  useEffect(() => {
    async function loadForm() {
      try {
        ...
        setError("Failed to load form. Please check your link.");
      } finally {
        setLoading(false);
      }
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    for (const q of formData.form.questions) {
      if (q.required) {
  ...
  if (loading) {
    return (
      <div className="text-center py-16 text-gray-400 text-sm">Loading form...</div>
    );
  }
  ...
  const { form, candidate_name } = formData;
  ...
  return (
    <div>
      <div className="rounded-xl border bg-white p-6 shadow-sm mb-6">
        ...
        {candidate_name && (
          <p className="text-gray-600 text-sm mb-2">Hi {candidate_name},</p>
        )}
        {form.description && (
